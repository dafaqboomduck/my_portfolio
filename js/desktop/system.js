...
function toggleShowDesktop() {
  ...
  if (allMinimized) {
    ...
    for (const id in windows) {
      if (!windows[id].minimized) minimizeWindow(id);
    }
  }
}
...
function toggleTheme() {
  ...
  if (typeof minesweeperGame !== 'undefined' && minesweeperGame)
    minesweeperGame.render();
  showNotification(
    darkTheme ? '🌙 Dark theme enabled' : '☀️ Light theme enabled',
  );
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  desktop.addEventListener('mousedown', (e) => {
    ...
    if (
      e.target.closest(
        '.vista-window, .taskbar, .start-menu, .vista-sidebar, .desktop-context-menu',
      )
    )
      return;
    ...
    marquee.rects = [...document.querySelectorAll('.desktop-icon')].map(
      (ic) => ({ el: ic, r: ic.getBoundingClientRect() }),
    );
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Enter') {
    if (e.target && e.target.id === 'startSearchInput') {
      const firstHit = document.querySelector(
        '#startSearchResults .start-search-hit',
      );
    ...
    if (sel) {
      const m = (sel.getAttribute('ondblclick') || '').match(
        /openWindow\(['"]([^'"]+)['"]\)/,
      );
      if (m) openWindow(m[1]);
    }
