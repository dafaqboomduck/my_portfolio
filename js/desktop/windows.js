...
function goBack(id) {
  ...
  suppressPush = true;
  closeWindow(id);
  if (previousWindow.startsWith('project-'))
    openProjectDetail(previousWindow.replace('project-', ''), stack);
  ...
  if (scrollPositions[previousWindow] != null && windows[previousWindow]) {
    const content =
      windows[previousWindow].element.querySelector('.window-content');
    if (content) {
