...
function homeHTML() {
  ...
  h += '</div>';
  h +=
    '<div class="home-foot">Windows Vista Portfolio · Phone Edition<br>' +
...
function aboutHTML() {
  ...
  b +=
    '<div class="card"><h4>Current status</h4><p>' +
    esc(a.status) +
    '</p></div>';
  b += '<h3>Education</h3>';
...
function projectsHTML() {
  ...
  for (const key in projectData) {
    ...
    b +=
      '<button class="row" data-go="project/' +
      key +
      '">' +
      '<span class="row-accent" style="background:' +
      color +
      '"></span>' +
      '<span class="row-main">' +
...
function projectDetailHTML(id) {
  ...
  p.achievements.forEach(function (x) {
    b += '<li>' + esc(x) + '</li>';
  });
  b += '</ul>';
  ...
  if (hasGit)
    b +=
      ...
      esc(p.github) +
      '" target="_blank" rel="noopener"><i class="bi bi-github"></i><span>View on GitHub</span></a>';
  if (hasDemo)
    b +=
      '<a class="btn-aero secondary" href="' +
      esc(p.demo) +
      '" target="_blank" rel="noopener"><i class="bi bi-play-circle"></i><span>Live demo</span></a>';
...
function skillsHTML() {
  ...
  b +=
    '<div class="card" style="margin-top:18px"><h4>Full technology stack</h4>' +
...
function techHTML() {
  let b = '';
  const cats = [];
  installedPrograms.forEach(function (p) {
  ...
  cats.forEach(function (cat) {
    ...
    installedPrograms
      ...
      .forEach(function (p) {
        b +=
          '<div class="tech-item"><i class="bi ' +
          p.icon +
          '"></i><div>' +
          '<div class="ti-name">' +
          esc(p.name) +
...
function resumeHTML() {
  ...
  b +=
    '<a class="btn-aero" href="' +
    url +
    '" target="_blank" rel="noopener"><i class="bi bi-box-arrow-up-right"></i><span>Open resume</span></a>';
  b +=
    '<a class="btn-aero secondary" href="' +
    url +
    '" download><i class="bi bi-download"></i><span>Download</span></a>';
...
function contactHTML() {
  ...
  b +=
    ...
    '<div><div class="cr-label">Email</div><div class="cr-value">' +
    esc(c.email) +
    '</div></div></a>';
  b +=
    '<a class="contact-row" href="' +
    esc(c.github) +
    '" target="_blank" rel="noopener"><i class="bi bi-github"></i>' +
    '<div><div class="cr-label">GitHub</div><div class="cr-value">' +
    esc(c.githubLabel) +
    '</div></div></a>';
