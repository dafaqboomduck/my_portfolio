...
function initMinesweeper() {
  ...
  document.getElementById('msDig').onclick = function () {
    msSetFlagMode(false);
  };
  document.getElementById('msFlag').onclick = function () {
    msSetFlagMode(true);
  };
...
function msReset() {
  ...
  for (let r = 0; r < MS.rows; r++) {
    ...
    for (let c = 0; c < MS.cols; c++) {
      msState.grid[r][c] = 0;
      msState.revealed[r][c] = false;
      msState.flagged[r][c] = false;
    }
  }
  ...
  document.getElementById('msMsg').textContent =
    'tap a tile to start · long-press to flag';
  msSetFlagMode(false);
  msRenderGrid();
  msUpdateMineCount();
}
...
function msPlaceMines(safeR, safeC) {
  ...
  for (let r = 0; r < MS.rows; r++) {
    for (let c = 0; c < MS.cols; c++) {
      if (msState.grid[r][c] === -1) continue;
      let n = 0;
      msNeighbors(r, c, function (rr, cc) {
        if (msState.grid[rr][cc] === -1) n++;
      });
      msState.grid[r][c] = n;
    }
  }
...
function msLose(br, bc) {
  ...
  for (let r = 0; r < MS.rows; r++) {
    for (let c = 0; c < MS.cols; c++) {
      if (msState.grid[r][c] === -1) {
        msState.revealed[r][c] = true;
        msRenderCell(r, c, r === br && c === bc);
      }
    }
  }
  document.getElementById('msFace').textContent = '😵';
  document.getElementById('msMsg').textContent =
    'boom — tap the face to try again';
}
...
function msWin() {
  ...
  document.getElementById('msFace').textContent = '😎';
  document.getElementById('msMsg').textContent =
    'cleared in ' + msState.time + 's — nice';
}

function msUpdateMineCount() {
  let flags = 0;
  for (let r = 0; r < MS.rows; r++)
    for (let c = 0; c < MS.cols; c++) if (msState.flagged[r][c]) flags++;
  document.getElementById('msMines').textContent = pad3(
    Math.max(0, MS.mines - flags),
  );
}
